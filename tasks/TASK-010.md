...
WIP 매트릭스(표)가 화면에 표시된 후, 사용자가 특정 제품과 공정 단계(셀)를 클릭하면 그 정보를 저장하는 기능입니다. 저장된 정보는 나중에 그 제품·공정의 상세 작업 내역(Lot) 팝업을 표시할 때 사용됩니다. 완료되면 사용자가 매트릭스에서 직접 셀을 선택할 수 있게 됩니다.

## 관련 파일

| 파일 경로 | 상태 | 이 태스크에서의 역할 |
|-----------|------|---------------------|
| `app.py` | 기존 파일 | st.dataframe on_select 이벤트 구현, session_state 저장 위치 |
| `data/aggregate.py` | 기존 파일 | 매트릭스 데이터 구조 참고 (인덱스=Product, 컬럼=STEP) |
| `tasks/TASK-011.md` | 의존 태스크 | 저장된 selected_product/step으로 Lot 조회 함수 구현 예정 |
